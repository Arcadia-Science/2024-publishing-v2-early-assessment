...
def analyze_publications(csv_path):
    try:
        ...
        df["Flesch reading ease version of record"] = (
            df["Flesch reading ease version of record"].str.rstrip("%").astype("float")
        )
        ...
        v1_workdays_median = (
            float(np.nanmedian(v1_completed_workdays))
            if not v1_completed_workdays.empty
            else np.nan
        )
        v1_workdays_mean = (
            float(np.nanmean(v1_completed_workdays))
            if not v1_completed_workdays.empty
            else np.nan
        )
        v1_workdays_std = (
            float(np.nanstd(v1_completed_workdays))
            if not v1_completed_workdays.empty
            else np.nan
        )

        # Calculate v1.0 Flesch reading ease statistics (only for non-empty values)
        v1_flesch = v1_df[v1_df["is_published"]][
            "Flesch reading ease version of record"
        ].dropna()
        v1_flesch_median = (
            float(np.median(v1_flesch)) if not v1_flesch.empty else np.nan
        )
        ...
        v2_workdays_completed_median = (
            float(np.nanmedian(v2_completed_workdays))
            if not v2_completed_workdays.empty
            else np.nan
        )
        v2_workdays_completed_mean = (
            float(np.nanmean(v2_completed_workdays))
            if not v2_completed_workdays.empty
            else np.nan
        )
        v2_workdays_completed_std = (
            float(np.nanstd(v2_completed_workdays))
            if not v2_completed_workdays.empty
            else np.nan
        )

        # Calculate v2.0 Flesch reading ease statistics (only for non-empty values)
        v2_flesch = v2_df[v2_df["is_published"]][
            "Flesch reading ease version of record"
        ].dropna()
        v2_flesch_median = (
            float(np.median(v2_flesch)) if not v2_flesch.empty else np.nan
        )
        ...
        v2_workdays_in_progress_median = (
            float(np.nanmedian(v2_in_progress_workdays))
            if not v2_in_progress_workdays.empty
            else np.nan
        )
        v2_workdays_in_progress_mean = (
            float(np.nanmean(v2_in_progress_workdays))
            if not v2_in_progress_workdays.empty
            else np.nan
        )
        v2_workdays_in_progress_std = (
            float(np.nanstd(v2_in_progress_workdays))
            if not v2_in_progress_workdays.empty
            else np.nan
        )
        ...
        v2_requests_median = (
            float(np.nanmedian(v2_requests)) if not v2_requests.empty else np.nan
        )
        v2_requests_mean = (
            float(np.nanmean(v2_requests)) if not v2_requests.empty else np.nan
        )
        v2_requests_min = (
            int(np.nanmin(v2_requests)) if not v2_requests.empty else np.nan
        )
        v2_requests_max = (
            int(np.nanmax(v2_requests)) if not v2_requests.empty else np.nan
        )
        v2_requests_std = (
            float(np.nanstd(v2_requests)) if not v2_requests.empty else np.nan
        )

        # Calculate v2.0 request statistics - Completed Pubs
        v2_requests_completed = v2_df[v2_df["is_completed"]][
            "Total number of pub team requests"
        ]
        v2_requests_completed_median = (
            float(np.nanmedian(v2_requests_completed))
            if not v2_requests_completed.empty
            else np.nan
        )
        v2_requests_completed_mean = (
            float(np.nanmean(v2_requests_completed))
            if not v2_requests_completed.empty
            else np.nan
        )
        v2_requests_completed_min = (
            int(np.nanmin(v2_requests_completed))
            if not v2_requests_completed.empty
            else np.nan
        )
        v2_requests_completed_max = (
            int(np.nanmax(v2_requests_completed))
            if not v2_requests_completed.empty
            else np.nan
        )
        v2_requests_completed_std = (
            float(np.nanstd(v2_requests_completed))
            if not v2_requests_completed.empty
            else np.nan
        )

        # Calculate v2.0 request statistics - Incomplete Pubs
        v2_requests_incomplete = v2_df[~v2_df["is_completed"]][
            "Total number of pub team requests"
        ]
        v2_requests_incomplete_median = (
            float(np.nanmedian(v2_requests_incomplete))
            if not v2_requests_incomplete.empty
            else np.nan
        )
        v2_requests_incomplete_mean = (
            float(np.nanmean(v2_requests_incomplete))
            if not v2_requests_incomplete.empty
            else np.nan
        )
        v2_requests_incomplete_min = (
            int(np.nanmin(v2_requests_incomplete))
            if not v2_requests_incomplete.empty
            else np.nan
        )
        v2_requests_incomplete_max = (
            int(np.nanmax(v2_requests_incomplete))
            if not v2_requests_incomplete.empty
            else np.nan
        )
        v2_requests_incomplete_std = (
            float(np.nanstd(v2_requests_incomplete))
            if not v2_requests_incomplete.empty
            else np.nan
        )
        ...
        print(
            f"Standard deviation of workdays to completion, v1.0: {v1_workdays_std:.1f}"
        )
        print(
        ...
        print(
            f"Min number of pub team requests, v2.0 (completed): "
            f"{v2_requests_completed_min}"
        )
        print(
            f"Max number of pub team requests, v2.0 (completed): "
        ...
        print(
            f"Min number of pub team requests, v2.0 (incomplete): "
            f"{v2_requests_incomplete_min}"
        )
        print(
            f"Max number of pub team requests, v2.0 (incomplete): "
