...
def group_other_impacts(
    impact_counts, grouping_method, threshold=None, other_impacts=None
):
    if grouping_method == "threshold":
        if threshold is None:
            raise ValueError(
                "Threshold must be provided for threshold grouping method."
            )
        common_impacts = (
        ...
        if other_impacts is None:
            raise ValueError(
                "List of impacts must be provided for manual grouping method."
            )
...
def set_impact_order(impact_counts, sort_version, other_position):
    ...
    ordered_percentages = impact_counts[
        impact_counts["Publishing version (from Pub)"] == sort_by_version
...
def print_statistics(impact_counts, original_data, expanded_data):
    ...
    for version in ["v1.0", "v2.0"]:
        version_data = original_data[
            original_data["Publishing version (from Pub)"] == version
        ]
        version_expanded = expanded_data[
            expanded_data["Publishing version (from Pub)"] == version
        ]
...
def create_plot(impact_counts, impact_pivot, versions, chart_type, chart_style="pie"):
    if chart_type == "bar":
        ...
        for i, version in enumerate(versions):
            offset = i * bar_width
            ax.bar(
                [p + offset for p in x], impact_pivot[version], bar_width, label=version
            )
        ...
        for category in wedge_order:
            if category in impact_pivot["Impact"].values:
                value = impact_pivot.loc[
                    impact_pivot["Impact"] == category, "v1.0"
                ].iloc[0]
        ...
        v1_total = int(
            round(
                impact_counts[impact_counts["Publishing version (from Pub)"] == "v1.0"][
                    "Count"
                ].sum()
            )
        )
        ...
        for category in wedge_order:
            if category in impact_pivot["Impact"].values:
                value = impact_pivot.loc[
                    impact_pivot["Impact"] == category, "v2.0"
                ].iloc[0]
                if pd.notna(value) and value > 0:
        ...
        v2_total = int(
            round(
                impact_counts[impact_counts["Publishing version (from Pub)"] == "v2.0"][
                    "Count"
                ].sum()
            )
        )
...
def main():
    ...
    total_impacts = impact_counts.groupby("Publishing version (from Pub)")[
        "Count"
    ].transform("sum")
    ...
    impact_counts = group_other_impacts(
        impact_counts, args.grouping, args.threshold, args.other_impacts
    )
    ...
    # Set impact order
    impact_counts = set_impact_order(
        impact_counts, args.sort_version, args.other_position
    )
    ...
    impact_pivot["Impact"] = pd.Categorical(
        impact_pivot["Impact"], categories=impact_order, ordered=True
    )
    ...
    fig = create_plot(
        impact_counts, impact_pivot, versions, args.chart_type, args.chart_style
    )
